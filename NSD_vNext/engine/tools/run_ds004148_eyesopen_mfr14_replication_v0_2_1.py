...
def _fit_channel_refusal_safe(signal: np.ndarray) -> tuple[np.ndarray, dict[str, object]]:
    """Preserve the fixed primary fit when the knee sensitivity fit refuses."""
    ...
    # Primary representation: refusal remains fatal.
    ...
    model_family_refused = False
    model_family_refusal_error_type = None
    model_family_refusal_error = None
    model_family_disagreement = False
    ...
    try:
        ...
        comparison = compare_aperiodic_models(
            fixed,
            knee,
            center_tolerance_hz=1.0,
        )
        model_family_disagreement = bool(comparison.flags)
    except Exception as exc:
        # Sensitivity-only refusal is retained explicitly, never imputed.
        model_family_refused = True
        model_family_refusal_error_type = type(exc).__name__
        model_family_refusal_error = str(exc)
    ...
    return np.log10(power), {
        ...
        "peaks": [
            {
                "center_frequency_hz": float(peak.center_frequency_hz),
                "power_above_aperiodic_log10": float(
                    peak.power_above_aperiodic_log10
                ),
                "descriptive_bandwidth_hz": float(peak.bandwidth_hz),
            }
            for peak in fixed.peaks
        ],
        "model_family_disagreement": model_family_disagreement,
        "model_family_refused": model_family_refused,
        "model_family_refusal_error_type": model_family_refusal_error_type,
        "model_family_refusal_error": model_family_refusal_error,
        "width_boundary_hit_count": int(
            diagnostics.lower_width_limit_hit_count
            + diagnostics.upper_width_limit_hit_count
        ),
...
def _recording(
    ...
    for label, signal in zip(labels, data):
        log_power, fitted = _fit_channel_refusal_safe(signal)
...
def _pair_compact(pair: dict[str, object]) -> dict[str, object]:
    ...
    refused = int(
        pair["cooccurrence"].get(
            "MODEL_FAMILY_REFUSED_within_PEAK_SHIFT_HIGH", 0
        )
    )
    return {
        ...
        "model_family_refused_within_high_count": refused,
        "model_family_refused_within_high_fraction": (
            refused / high if high else None
        ),
...
def execute(
    ...
    result = {
        "schema": "NSD_DS004148_EYESOPEN_MFR14_REPLICATION_RESULT_V0_2_1",
        "status": status,
        ...
        "interpretation_firewall": [
            ...
            "knee sensitivity refusal is retained explicitly and is not imputed or treated as model-family agreement",
    ...
    if not failures:
        ...
        for pair_name in pair_names:
            aggregate[pair_name] = {
                ...
                "model_family_refused_within_high_fraction": _summary([
                    subject["pairs"][pair_name][
                        "model_family_refused_within_high_fraction"
                    ]
                    for subject in subject_results
...
def main() -> int:
    ...
    result_path = args.output_dir / "ds004148_eyesopen_mfr14_replication_result_v0.2.1.json"
