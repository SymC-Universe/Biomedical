#!/usr/bin/env python3
"""Audit ds004148 eyes-open BrainVision header topology across the full dataset.
...
def execute(matched_reference_path: Path, output_dir: Path) -> dict[str, object]:
    ...
    return {
        "schema": "NSD_DS004148_EYESOPEN_HEADER_TOPOLOGY_AUDIT_V0_1",
        "status": (
            "HEADER_AUDIT_CLOSED"
            if not failures
...
def _write_summary(result: dict[str, object], path: Path) -> None:
    lines = [
        "# ds004148 eyes-open dataset-wide header topology audit",
        "",
...
def main() -> int:
    ...
    result_path = (
        args.output_dir
        / "ds004148_eyesopen_header_topology_audit_v0.1.json"
    )
