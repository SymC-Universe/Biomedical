...
for vi=1:numel(validationNames)
    scenario=validationNames(vi);
    ith=find(scenarios==scenario,1,'first');
    if isempty(ith), error('BIO_CHI_D2FC2_MISSING_VALIDATION_SCENARIO_%s',scenario); end
    ...
    validation(vi).scenario=char(scenario);
    validation(vi).n_points=n;
    validation(vi).range=yrange;
    validation(vi).peak_count=numel(locs);
    validation(vi).peak_times_min=tt(locs)';
    validation(vi).interpeak_min=periods';
    validation(vi).median_period_min=periodMed;
    validation(vi).omega_rad_per_min=omegaEmp;
    validation(vi).identifiable=logical(identifiable);
end
