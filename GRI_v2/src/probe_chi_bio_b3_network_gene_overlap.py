...
"""Outcome-free B3 network / GSE98812 identifier-support qualification.

This probe is deliberately limited to exact source identities and gene symbols.
It never reads a GSE98812 expression value, never scores TF activity, never
selects a state dimension from expression, and never fits an operator.

Prospective eligibility rule fixed before execution:
  * exact case-sensitive target-symbol match to the GSE98812 GENE universe;
  * at least 5 distinct measured targets per regulator, matching ULM tmin=5;
  * CollecTRI uses the full canonical signed human network;
  * DoRothEA sensitivity uses confidence A/B/C only;
  * DoRothEA prospective weights are mor / {A:1, B:2, C:3}, but this probe
    inventories eligibility only and does not score those weights.
"""
...
def _gse_gene_universe(payload: bytes) -> list[str]:
    ...
    # Deliberately parse ONLY the identifier column. Expression values are not
    # opened by this qualification step.
    ...
    if genes.isna().any() or genes.eq("").any() or genes.duplicated().any():
        raise RuntimeError("GSE98812 GENE universe contains missing/duplicate identifiers")
    return genes.tolist()
...
def run_probe(output_dir: Path) -> dict:
    ...
    result = {
        "status": "PASS_B3_NETWORK_GENE_OVERLAP_PROVENANCE_ONLY",
    ...
    path = output_dir / "GRI_CHI_BIO_B3_NETWORK_GSE98812_OVERLAP.json"
    path.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return result


def main() -> int:
    result = run_probe(Path("development_outputs/chi_bio_regulon_source/b3_overlap_20260913"))
