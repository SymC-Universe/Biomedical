...
GRI remains a **development-stage cross-omic architecture / audit-refusal program**, not a validated clinical cancer tool.
...
The following are closed in the current internal lineage:

- Stage A / A1.1 static RNA architecture and fixed-n calibration;
- Stage B1 composition/context decomposition;
- Stage B2 RPPA and genomic integration;
- Tool feasibility F1-F4, with **F4 = NARROW**;
...
- `STAGE_C1_COMPLETE`;
- implementation `stage-c1-frozen-v2-exec-20260906.3`;
- 32 cancers;
- 100 deterministic n=30 resamples per cancer;
- 9,457 shared eligible samples;
- exact historical C1A recovery;
- C1-1 PASS;
- C1-2 PASS;
- C1-3 PASS;
- highest promotion `C1-3`;
...
Primary internal results:

- 18 primary-evaluable cancers;
- ALL_METHYLATION_RIDGE median normalized MSE 0.4440;
- COVARIATE_ONLY median normalized MSE 0.7018;
- ALL_METHYLATION_RIDGE beat COVARIATE_ONLY in 17/18 cancers;
- sole loss PCPG;
- exact one-sided sign-test p = 7.2479248046875e-05;
- D4-informed global-geometry confidence ranked lower FINAL_HOLDOUT risk with rho = -0.56037 and one-sided 99,999-permutation p = 0.00795;
- AURC = 0.4026622, one-sided p = 0.00526.

This is internal TCGA evidence. The 47-predictor versus two-covariate comparison establishes additional methylation information beyond those two covariates, not a capacity-matched Hallmark-architecture advantage.
...
Structural comparator work is already meaningful:

- simple baselines constrain several synthetic contrasts;
- AJIVE and MOFA2 recover much of global shared/private structure;
- global CKA and modal shared/private decomposition are therefore treated as established primitives rather than standalone novelty claims;
- DIVAS remained `NOT_EVALUABLE` under the pinned published implementation.

The exact previously referenced **post-FINAL equal-dimensional predictive capacity-control package has not yet been recovered** from the repository or File Library. References to the control exist in the manuscript/reviewer record, but no exact runnable freeze identity has been verified.
...
- static multiomic architecture under the frozen C1 lineage;
- methylation organization beyond construction-aware nulls;
- broad static methylation/RNA patient-geometry alignment;
...
- temporal substrate inheritance or recovery;
- treatment response;
...
1. return and audit post-C1 sensitivity v2.2;
2. resolve capacity-matched comparator provenance/execution status;
