...
4. **External permutation precision: ACTIVE.** Frozen B=9,999 deterministic extension preserves the original B=999 stream and cannot change original P1 classification. Initial run failed mechanically before statistics because of an incorrect support path; repaired workflow now uses the exact original Hallmark acquisition and support payload. Active/successor run must be audited when terminal.
...
6. **Standard-toolkit / technical comparator: ACTIVE MATCHED ATTACK.** A composition+TCGA-TSS-preserving H1 null is frozen and executing. It also reports ordinary PCA PC1 variance fraction against the identical stronger null as a conventional within-layer comparator. MOFA/SNF/DIABLO will not be run merely for optics because they do not answer the exact H1 construction question and no superiority claim is made over them.
...
### Additional active H1 technical gate

`BIOSYSTEMS_ADVERSARIAL_R1_H1_TSS_FREEZE_20260923.md` prospectively freezes a post-result sensitivity preserving linear purity/leukocyte effects plus TCGA Tissue Source Site mean structure. The same run reports standard PCA PC1 variance fraction as a secondary conventional comparator. This gate may narrow H1 further but cannot retroactively promote C1.
