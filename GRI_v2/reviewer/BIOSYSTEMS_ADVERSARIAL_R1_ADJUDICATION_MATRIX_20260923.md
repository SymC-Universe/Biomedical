...
| H1 lacks batch/center/plate/age/sex attack | **PARTLY VALID / TESTABLE SOURCE STRUCTURE ATTACKED** | External metadata audit found no additional identifiable prostate technical covariate beyond race. Internally, a post-result H1 null preserving purity/leukocyte plus TCGA Tissue Source Site mean structure remains positive in 30/30 cancers (median +0.08162, exact p=1.86e-9). Plate/slide/batch variables not exposed by the frozen public lineage remain unresolved. |
...
| B=999 makes p=0.001/q=0.003 the Monte Carlo resolution floor | **VALID / CLOSED BY PRECISION TEST** | Exact deterministic extension to B=9,999 preserves status: H1 p=0.0001/BH q=0.0003 on both primary platform lanes; H2/H3a remain unresolved. Original P1 decision unchanged. |
...
| Round-1 issue | Adjudication | Evidence / action |
|---|---|---|
| No benchmark against MOFA/SNF/DIABLO/CCA | **PARTLY VALID / MATCHED CONVENTIONAL COMPARATOR ADDED** | These tools answer different latent-factor/fusion/supervised questions than the exact H1 construction-null task. A matched ordinary PCA PC1-fraction comparator was therefore evaluated against the same composition+TSS-preserving null and is positive 30/30 cancers. The paper makes no superiority claim over MOFA/SNF/DIABLO. |
