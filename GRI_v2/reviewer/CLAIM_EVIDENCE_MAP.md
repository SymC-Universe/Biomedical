...
| Genomic decomposition | Five documented genomic coordinates are being tested separately against the frozen RNA map, with a same-patient permuted-coordinate construction null and an `INCREMENT_B1` branch | In progress, no biological result yet | `../config/stage_b2_integration_plan.json` | `../src/run_stage_b2_genomic.py`, `../src/run_stage_b2_genomic_resume.py`; genomic tests | final outputs pending | final audit pending |
...
- partial Stage B2 genomic checkpoints are execution state, not evidence;
