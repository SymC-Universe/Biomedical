...
**SCIENTIFIC_STATE_CHANGED:** YES at candidate-family level by explicit user approval; `Chi_bio` itself remains `NOT_ADMITTED`.

## APPROVED SCIENTIFIC FAMILY

The user explicitly approved the recommended plan on 2026-09-13:

- primary family: G1 normalized regulatory-interaction;
...
- scope: L3 paired local + embedded;
...
- general directed heterogeneous restoration: naive `alpha(R^-1 K) = 1` route rejected by exact false-safe/false-unsafe counterexamples.

Therefore an empirical transcriptomic G1 requires independently justified restoration structure. Static covariance/CKA/H2/H3 objects are not substitutes for the generator.
...
- A3: require material conclusions to agree across both, otherwise `REPRESENTATION_DEPENDENT_NO_TRANSFER`;
- A4: another prospectively justified outcome-blind rule.

No real G2 trajectory should be computed until this is frozen.
...
Current evidence supports an approved and hardened G1-S1-L3 candidate-family program, mathematically valid exact-unity subclasses, explicit failure routes, and a tightly constrained empirical path. It does **not** support an admitted biological Chi, a numerical cancer Chi_bio map, or a biological `Chi_bio = 1` boundary.