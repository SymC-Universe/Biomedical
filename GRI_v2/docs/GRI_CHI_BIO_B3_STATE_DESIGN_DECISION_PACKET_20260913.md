# GRI Chi_bio B3 state-design decision packet

**Date:** 2026-09-13  
**Mode:** P0-D pre-outcome science-decision preparation  
**Chi_bio status:** `NOT_ADMITTED`  
**Purpose:** expose the next scientifically material B3 choice without selecting it from SCC25/TCGA outcomes.

## 1. Evidence already earned before this decision

The B3 source/provenance program now establishes the following without opening expression outcomes:

- CollecTRI and DoRothEA exact source identities are frozen and byte/provenance qualified;
- GSE98812 raw `GENE` identifiers are `SYMBOL|ENTREZID`;
- deterministic symbol mapping leaves 20,502 unique mapped symbols after excluding 29 literal `?` rows and applying two Entrez-backed historical `SLC35E2` disambiguations;
- under the predeclared minimum of five measured targets per regulator, CollecTRI supports 766 regulators and DoRothEA A/B/C supports 271;
- the eligible-regulator intersection contains 249 regulators;
...
For the existing unregularized designs:
...
A necessary row-count condition for full column rank is therefore:
...
The D2 ceiling of eight is **not** a recommended dimension. It is only a necessary algebraic ceiling. Conditioning, predictive adequacy, representation stability, modal fidelity and scientific semantics can force a lower effective dimension.
...
1. **What is each B3 state coordinate?**
   - signed-regulon TF-activity score;
   - signed module-expression score;
   - another explicitly derived regulatory coordinate.
2. **How is the state reduced to a supportable dimension?**
   - externally fixed small panel;
   - full externally defined eligible universe followed by a prospectively fixed outcome-blind reduction;
   - another independently justified rule.
...
Use an external evidence rule to choose a small panel before any SCC25/TCGA expression is opened.

**Strengths:** maximal interpretability and direct transport.  
**Risks:** panel selection imports a strong biological prior and may omit relevant modes.  
**Required safeguard:** the external rule must be fully specified and independent of current cancer outcomes.

### B3-B: full qualified network universe plus fixed scoring and outcome-blind reduction
...
Retain a larger regulatory state and introduce an independently justified sparse/shrinkage operator estimator.

**Strengths:** potentially retains more regulatory structure.  
...
- the reduction destroys dominant modal/non-normal structure beyond the frozen tolerance;
- transport requires cancer-specific semantic redefinition;
...
- selecting the exact B3 state-scoring semantics;
- selecting the final regulator panel or reduction rule;
- opening treatment outcomes to choose that representation;
- executing normalized empirical G1 under C3;
- interpreting the mathematical unit boundary as an admitted biological `Chi_bio = 1` boundary.
