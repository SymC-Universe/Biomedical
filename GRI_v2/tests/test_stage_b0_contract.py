...
def test_barcode_scanner_counts_hyphen_and_dot_formats():
    with tempfile.TemporaryDirectory() as td:
        ...
        p.write_text(
            ...
            "TCGA.CC.9999.01A\t0.7\n"
        )
        d = scan_barcodes(p)
        assert d["unique_tcga_patients_detected"] == 3
        assert d["unique_tcga_sample_barcodes_detected"] == 4
        assert d["unique_primary_tumor_sample_barcodes_detected"] == 3
