...
O=R/'results'/'phase0d_v01'; O.mkdir(parents=True,exist_ok=True)
L=O/'RUN_LOG.txt'
def log(s): print(s,flush=True); open(L,'a',encoding='utf-8').write(s+'\n')
def cmd(a): return subprocess.run(a,cwd=R).returncode
if __name__=='__main__':
    L.write_text('')
    log('NSD Phase 0D v0.1 untouched local-order three-layer synthetic holdout runner')
    log('Full-record mixture order is diagnostic only; local segments choose order independently.')
    log('Scalar-spectrum, modal/subspace, and crowding-aware system-organization gates are frozen separately.')
    ...
    for a in range(1,4):
        ...
        rc=cmd([sys.executable,'run_phase0d.py'])
        if rc==0:
        ...
        time.sleep(5)
    log('RUN DID NOT COMPLETE. Preserve results/phase0d_v01 and send the entire folder.'); raise SystemExit(1)
