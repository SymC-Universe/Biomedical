...
CFG=json.loads((ROOT/'configs'/'phase0d_design.json').read_text())
...
def _toy(global_scale=1.0, rotate=False, structural=False, crowding=False, single=False):
    ...
    # Full record deliberately looks higher order than either local segment.
    decomps={'full':_decomp_gap(8),'first_half':_decomp_gap(4),'second_half':_decomp_gap(4)}
    return fits,decomps,ORDERS


def test_phase0d_rules_lock_local_order_and_carrier_matching():
...
def test_holdout_is_new_and_marked_unexecuted_by_design():
    assert CFG['seed']==202609081703
    assert CFG['durations_samples']==[12000,22000,30000]
    assert CFG['candidate_orders']==[2,4,6,8,10,12,14,16]
    assert CFG['epistemic_status']=='UNTOUCHED_PHASE0D_LOCAL_ORDER_HOLDOUT_DO_NOT_EXECUTE_DURING_DEVELOPMENT'
...
def test_single_complex_mode_can_admit_system_with_geometry_unresolved():
    ...
    g=evaluate_system_layer(fits,decomps,orders,RULES)
    assert g['decision']=='ADMIT_SYSTEM_ORGANIZATION_GEOMETRY_UNRESOLVED'
...
def test_nonnormal_family_is_stable_truth():
    spec=next(x for x in CFG['systems'] if x['name']=='d01_nonnormal_cond55')
    A,C=make_linear_system(spec,CFG['n_channels'],np.random.default_rng(9))
    assert A.shape==(4,4)
    assert np.all(np.linalg.eigvals(A).real<0)


def test_layer_challenges_are_scored_only_at_exact_switch_split():
    for spec in CFG['systems']:
        if spec.get('role')=='layer_challenge':
            assert spec['score_only_n_samples']==[30000]
            assert spec['switch_sample']==15000


def test_two_distinct_null_families_are_frozen():
    nulls=[x for x in CFG['systems'] if x['role']=='null']
    assert len(nulls)==2
    assert sorted(x['beta'] for x in nulls)==[0.95,1.35]
...
def test_no_clinical_inputs_in_holdout_design():
    txt=(ROOT/'configs'/'phase0d_design.json').read_text().lower()
