...
import hashlib
R=Path(__file__).resolve().parents[1]; O=R/"results"/"phase0c"; O.mkdir(parents=True,exist_ok=True)
inc=[]
for rr in ["src","configs","tests","scripts"]:
    for p in sorted((R/rr).rglob("*")):
        if p.is_file() and "__pycache__" not in p.parts: inc.append(p)
for n in ["README_FIRST.md","PREREGISTRATION.md","PRE_RUN_FREEZE.json","run_phase0c.py","local_runner.py",".gitignore"]:
    p=R/n
