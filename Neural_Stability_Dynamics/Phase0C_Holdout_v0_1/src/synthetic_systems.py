...
def simulate_switch_fixed(spec,n_channels,dt,n_samples,process_scale,rng):
    ...
    A20,_=build_canonical(spec["modes_second"])
    from scipy.linalg import schur
    _,Q=schur(A1,output="real")
    A2=Q@A20@Q.T
    F1=expm(A1*dt); F2=expm(A2*dt)
    n=A1.shape[0]; x=np.zeros(n); Y=np.empty((n_samples,n_channels))
    burn=max(2000,int(20/dt))
    for _ in range(burn):
        x=F1@x+process_scale*np.sqrt(dt)*rng.normal(size=n)
    sw=int(spec["switch_sample"])
    if sw<=0 or sw>=n_samples:
        raise ValueError("switch_sample must fall inside generated record")
    for k in range(n_samples):
