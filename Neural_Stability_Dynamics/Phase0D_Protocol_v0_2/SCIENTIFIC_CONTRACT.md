# NSD Phase 0D v0.2 P0 Scientific Contract
...
8. Scalar spectral state, modal/carrier structure and conglomerate/system organization are starting representations, not a complete decomposition of chi and not an arithmetic combination.
9. A whole-system scalar is prohibited unless separately derived and validated.
10. Layer C subclaims are reported separately. Unresolved relational geometry is `UNRESOLVED` and yields only a PARTIAL system result, not a full admission. Claim-specific gates may not suppress a narrower result that does not logically depend on them.
11. Local order change is preserved as open-channel structure. Until a shared-mode persistence method is justified, unequal-order comparisons are not forced into a false full-system interpretation.
12. Numeric thresholds inherited from v0.1 or developed during P0 are method-development/calibration values only unless separately frozen under the applicable prospective record. They are not P1-frozen merely because they perform well in P0.
13. No new uncertainty margin, model-adequacy threshold, comparator or regime boundary will be invented for convenience.
14. Function Map and Limit Map are coequal P0 scientific targets where native/synthetic method science permits them. The project maps where a representation works, where it degrades/refuses/fails, and what remains unknown; no single aggregate validity score replaces these maps.
15. Substantial P0 work seeks coverage across `NOMINAL_FUNCTION`, `PERTURBED_FUNCTION`, `BOUNDARY_OR_TRANSITION`, and `RARE_NATURAL_LIMIT` where applicable. `NOT_APPLICABLE`, `NOT_AVAILABLE`, and `UNRESOLVED` are valid states; the current synthetic stage does not manufacture a rare-natural role.
16. Rare natural cases, if used later, require domain-native rarity, authenticity/confounding checks, base-rate context, and an outcome-independent eligibility/selection route for P1. Rare does not mean representative.
