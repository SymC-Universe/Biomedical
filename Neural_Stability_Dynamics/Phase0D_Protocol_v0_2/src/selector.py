...
def evaluate_scalar_layer(fits,decomps,candidate_orders,rules):
    ...
    if q1!=q2:
        out['order_transition_status']='ORDER_CHANGED_SHARED_MODE_TRACKING_UNRESOLVED'
...
def evaluate_system_layer(fits,decomps,candidate_orders,rules):
    ...
    carrier_ok=split['carrier_ok'] and co1['carrier_ok'] and co2['carrier_ok']
    participation_ok=(
        np.isfinite(split['participation_TV']) and split['participation_TV']<=float(sr['split_channel_participation_TV_max']) and
        np.isfinite(co1['participation_TV']) and co1['participation_TV']<=float(sr['cross_order_channel_participation_TV_max']) and
        np.isfinite(co2['participation_TV']) and co2['participation_TV']<=float(sr['cross_order_channel_participation_TV_max'])
    )
    geometry_resolved=[x['relational']['status']=='RESOLVED' for x in [split,co1,co2]]
    geometry_unresolved=[x['relational']['status'].startswith('UNRESOLVED') for x in [split,co1,co2]]
    geometry_ok=split['geometry_ok'] and co1['geometry_ok'] and co2['geometry_ok']
    out['carrier_status']='ADMIT' if carrier_ok else 'REFUSE'
    out['participation_status']='ADMIT' if participation_ok else 'REFUSE'
    if all(geometry_resolved) and geometry_ok:
        out['geometry_status']='ADMIT'
    elif geometry_ok and any(geometry_unresolved):
        out['geometry_status']='UNRESOLVED'
    else:
        out['geometry_status']='REFUSE'
    if out['carrier_status']=='ADMIT' and out['participation_status']=='ADMIT' and out['geometry_status']=='ADMIT':
        out['decision']='ADMIT_SYSTEM_ORGANIZATION'
    elif out['carrier_status']=='ADMIT' and out['participation_status']=='ADMIT' and out['geometry_status']=='UNRESOLVED':
        out['decision']='PARTIAL_SYSTEM_ORGANIZATION_GEOMETRY_UNRESOLVED'
    else:
